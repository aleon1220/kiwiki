...
- [Security and chryptography](#security-and-chryptography)
  - [Security and Chryptography](#security-and-chryptography-1)
      ...
      - [Encrypt 2048 bits | Upgraded to 4096 bits](#encrypt-2048-bits--upgraded-to-4096-bits)
- [Encryption Concepts](#encryption-concepts)
  ...
  - [OpenSSL](#openssl)
    ...
    - [Encrypt text files](#encrypt-text-files)
      - [Add Certificates to Linux OS Key store](#add-certificates-to-linux-os-key-store)
...
#### Encrypt 2048 bits | Upgraded to 4096 bits
...
- `.p12` – `PKCS#12`, may contain certificate(s) (public) and private keys (password protected). Evolved from the personal information exchange `(PFX)` standard and is used to exchange public and private objects in a single file
- `X.509` Certificate - cryptography, X.509 is a standard defining the format of public key certificates
...
* If you have multiple SSH keys and want to use a specific one for a particular repository, you can set it up directly in the repository configuration.
...
```bash
ssh -T git@github.com
load pubkey "/home/aleonrangel/.ssh/id_rsa_personal": Permission denied
Hi aleon1220! You've successfully authenticated, but GitHub does not provide shell access.
```

* add the identity

```bash
ssh-add -k /home/aleonrangel/.ssh/id_rsa_personal
